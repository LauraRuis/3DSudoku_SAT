...
conflict_file = open("conflicts_count_3d.txt", "w")
...
for h in range(AMOUNT):
	try:
		# Get sudoku file and properties
		f = open(args.sudokufile + str(h) + ".txt")	
		print args.sudokufile + str(h) + ".txt"
		order, m = map(int, f.readline().split()[1:3])
		n = order / m
		sudlist = [f.readline().split() for i in range(order*order)]
		sudoku_list = []
		i = 0
		while i<(order*order):
			sudoku_list.append(sudlist[i:i+order])
		 	i+=order


		# Encode Sudoku to Dimacs CNF Formula
		initial_time = time.clock()
		s=sudoku3d.sudoku3d(args.sudokufile + str(h) + ".txt")
...
def print_solution(solution):
	...
	for var in map(int,solution):
		if var > 0:
			...
			if counter % (n*(n*m)) == 0:
				print "--------------------- |\n|",
			if counter % (order**2)  == 0:
				print "xxxxxxxxxxxxxxxxxxxxx |\n|",
			if counter >= order**3: break
...
def execute_crypto(h):
	...
	if 'SATISFIABLE' in output:
		...
		total_conflicts += int(output[conflict_indices[0] + 2])
